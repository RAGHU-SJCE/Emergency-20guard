...
export class LocationService {
  ...
  async reverseGeocode(
    ...
    try {
      if (!this.googleMapsApiKey) {
        console.warn(
          "Google Maps API key not configured, using fallback geocoding",
        );
  ...
  private isInUSA(coordinates: LocationCoordinates): boolean {
    ...
    return (
      (latitude >= 24.396308 &&
        latitude <= 49.384358 && // Continental US
        longitude >= -125.0 &&
        longitude <= -66.93457) ||
      (latitude >= 18.91619 &&
        latitude <= 28.402123 && // Hawaii
        longitude >= -178.334698 &&
        longitude <= -154.806773) ||
      (latitude >= 51.209464 &&
        latitude <= 71.406235 && // Alaska
        longitude >= -179.148909 &&
        longitude <= -129.979506)
    );
  }
  ...
  private fallbackReverseGeocode(latitude: number, longitude: number): string {
    ...
    const cities = [
      { name: "New York, NY", lat: 40.7128, lng: -74.006, radius: 0.5 },
      ...
      { name: "Phoenix, AZ", lat: 33.4484, lng: -112.074, radius: 0.5 },
    ];
