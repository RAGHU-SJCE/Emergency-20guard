...
export function EmergencyStatus({ 
  ...
  const statusItems = [
    ...
    },
    {
      id: 'location',
      label: 'Location',
      value: deviceFeatures.geolocation.permission === 'granted' ? 'Active' : 'Needs Permission',
      detail: null,
      icon: MapPin,
      status: deviceFeatures.geolocation.permission === 'granted' ? 'safe' : 'warning'
    },
    {
      id: 'notifications',
      label: 'Notifications',
      value: deviceFeatures.notifications.permission === 'granted' ? 'Enabled' : 'Needs Permission',
      detail: null,
      icon: Bell,
      status: deviceFeatures.notifications.permission === 'granted' ? 'safe' : 'warning'
    },
    {
      id: 'emergency',
    ...
    return (
      <div className={`flex items-center space-x-2 ${className}`}>
        {statusItems.slice(0, 3).map((item) => {
          ...
          return (
            <div key={item.id} className={`w-8 h-8 rounded-full flex items-center justify-center ${getStatusColor(item.status)}`}>
        ...
        {showPermissionButton && (
          ...
          <Button size="sm" variant="outline" onClick={requestPermissions} className="ml-2">
            Enable
          </Button>
        )}
      </div>
        ...
        {statusItems.map((item) => {
          ...
          return (
            <div key={item.id} className="text-center">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2 border-2 ${getStatusColor(item.status)} ${getStatusIcon(item.status)}`}>
      ...
      {emergencyServices.isEmergencyActive && (
        <div className="mt-4 p-3 bg-emergency/10 border border-emergency/20 rounded-lg">
          <div className="flex items-center justify-center text-center">
            ...
            <span className="text-emergency font-medium text-sm">Emergency Mode Active</span>
          </div>
        </div>
      )}
    </div>
