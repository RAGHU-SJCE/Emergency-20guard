...
export default function EmergencyHistory() {
  ...
  useEffect(() => {
    if (searchTerm.trim() === "") {
      ...
      const filtered = emergencyEvents.filter(event =>
        event.type.toLowerCase().includes(searchTerm.toLowerCase()) ||
        event.notes.toLowerCase().includes(searchTerm.toLowerCase()) ||
        event.location.address?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        event.contacts.some(contact => contact.toLowerCase().includes(searchTerm.toLowerCase()))
      );
  ...
  const convertToCSV = (events: EmergencyEvent[]): string => {
    const headers = ['Date', 'Time', 'Type', 'Location', 'Status', 'Response Time', 'Contacts', 'Notes', 'Call ID'];
  ...
  const stats = [
    { label: "Total Emergencies", value: emergencyEvents.length.toString(), icon: AlertTriangle },
    { 
      label: "Avg Response Time", 
      value: emergencyEvents.length > 0 
    ...
    { 
      ...
      value: emergencyEvents.reduce((total, event) => total + event.contacts.length, 0).toString(), 
      icon: Phone 
    }
  ];
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="text-xl text-slate-600 max-w-2xl">
                  View a comprehensive log of your real emergency events, response times, and incident details.
                </p>
              </div>
          ...
          {filteredEvents.length > 0 ? (
                    ...
                    <div className="flex-1">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                            ...
                            <div className="flex items-center text-slate-600">
                              ...
                              <span className="text-sm">
                                {event.location.address || `${event.location.latitude.toFixed(4)}, ${event.location.longitude.toFixed(4)}`}
                              </span>
                            </div>
                            <div className="flex items-center text-slate-600">
                          ...
                          {event.callId && (
                            <div className="mb-4">
                              <span className="text-sm font-medium text-secondary">Call ID: </span>
                              <span className="text-sm text-slate-600 font-mono">{event.callId}</span>
                            </div>
                          )}
            ...
            <div className="text-center py-16">
              ...
              <h2 className="text-2xl font-semibold text-secondary mb-4">
                {emergencyEvents.length === 0 ? "No Emergency History" : "No Matching Events"}
              </h2>
          ...
          <div className="mt-12 bg-gradient-to-r from-primary/5 to-secondary/5 rounded-xl p-6 lg:p-8">
            <div className="text-center">
              ...
              <p className="text-slate-600 mb-6 max-w-2xl mx-auto">
                ...
                All data is exported with timestamps and location information.
              </p>
